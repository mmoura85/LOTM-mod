...
export class TwilightGiantMenus {
  ...
  static async showDawnPaladinMenu(player, DawnPaladinSequence) {
    ...
    const selected  = DawnPaladinSequence.getSelectedAbility(player);
    const abilities = DawnPaladinSequence.getAllAbilities(player);

    const form = new ActionFormData()
      .title('§6⚔ Dawn Paladin')
      .body(
        `§7Spirit: §b${spirit}§7/§b${maxSpirit}\n` +
        `§7Selected: §e${abilities.find(a => a.id === selected)?.name ?? '§8none'}§r\n\n` +
        `§7Select to set active. Right-click (no sneak) to use.`
      );

    for (const ability of abilities) {
      const isActive = ability.id === selected;
      const marker   = isActive ? '§a✓ ' : '§7';
      form.button(`${marker}${ability.name}\n§b${ability.cost} Spirit`, 'textures/items/dawn_item');
    }
    form.button('§7Cancel', 'textures/ui/cancel');

    const response = await form.show(player);
    if (response.canceled) return;
    if (response.selection >= abilities.length) return;

    const chosen = abilities[response.selection];
    DawnPaladinSequence.setSelectedAbility(player, chosen.id);
    player.sendMessage(`§6Active ability: §e${chosen.name}`);
