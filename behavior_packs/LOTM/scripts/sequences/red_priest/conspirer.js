...
const BASE_SPIRIT     = 150;
const DANGER_RADIUS   = 28;
const TRAP_RADIUS     = 8;
const SCAN_INTERVAL_MS      = 2000;
const TRAP_SCAN_INTERVAL_MS = 1000;

// Upgraded spell damage (×2.5 of Pyromaniac)
const SPELLS = {
    fire_bolt:        { cost: 8,  cooldownMs: 1800,  label: '§cFire Bolt §7(8 sp)',         damage: 35  },
    fireball:         { cost: 18, cooldownMs: 3500,  label: '§6Fireball §7(18 sp)',          damage: 62, aoe: 5 },
    fire_wall:        { cost: 20, cooldownMs: 5000,  label: '§eFire Wall §7(20 sp)',         damage: 5   },
    blazing_spear:    { cost: 14, cooldownMs: 3500,  label: '§cBlazing Spear §7(14 sp)',     damage: 50  },
    ring_of_fire:     { cost: 25, cooldownMs: 7000,  label: '§4Ring of Fire §7(25 sp)',      damage: 40  },
    flaming_barrage:  { cost: 22, cooldownMs: 9000,  label: '§6Flaming Barrage §7(22 sp)',   damage: 20  },
    burning_dash:     { cost: 18, cooldownMs: 6000,  label: '§4Burning Dash §7(18 sp)',      damage: 20  },
    fire_arrow_blaze: { cost: 20, cooldownMs: 7000,  label: '§6Fire Arrow Storm §7(20 sp)',  damage: 55  },
    // New abilities
    flame_transform:  { cost: 20, cooldownMs: 10000, label: '§6Flame Transform §7(20 sp)'   },
    incitement:       { cost: 25, cooldownMs: 5000,  label: '§cIncitement §7(25 sp)'        },
    conspiracy:       { cost: 22, cooldownMs: 12000, label: '§5Conspiracy §7(22 sp)'        },
    conjure_blade:    { cost: 30, cooldownMs: 60000, label: '§4Conjure Flame Blade §7(30 sp)'},
};
...
const activeRings     = [];
const activeIncitements = new Map(); // playerId → intervalId
...
export class ConspirierSequence {
    ...
    static applyPassiveAbilities(player) {
        ...
        _applyEffect(player, 'strength', 2);     // Strength III
        _applyEffect(player, 'speed', 2);         // Speed III
        _applyEffect(player, 'jump_boost', 2);    // Jump Boost III
        _applyEffect(player, 'regeneration', 0);
        _applyEffect(player, 'health_boost', 3);  // +4 hearts
        _applyEffect(player, 'resistance', 0);    // Resistance I — enhanced physique
    ...
    // Enhanced melee: 8s burn + resistance debuff
    static onMeleeHit(player, victim) {
        try { victim.setOnFire(8, true); } catch (_) {}
        try { victim.addEffect('resistance', 60, { amplifier: 0, showParticles: false }); } catch (_) {}
        // Bonus damage on incandescent white flames
        try { victim.applyDamage(4, { damagingEntity: player }); } catch (_) {}
    }

    static async openSpellMenu(player) {
        const form = new ActionFormData()
            .title('§4§lConspirer Abilities')
    ...
    static tickEffects() {
        ...
        for (let i = activeRings.length - 1; i >= 0; i--) {
            ...
            const radius = (now - ring.startTime) / 1000 * 5; // 5 blocks/sec (faster than Pyromaniac)
...
// Color-coded entity detection:
//   Green  = animals / passive
//   Yellow = standard vanilla hostiles
//   Red    = dangerous LOTM mobs
//   Purple = beyonder-tier entities (soldiers, clowns, etc.)
//   Flame  = rampager / voidwatcher tier
...
function _castIncitement(player) {
    ...
    const RANGE = 26, DURATION_TICKS = 300; // 15 seconds
    let ticksLeft = DURATION_TICKS;

    // Immediately incite
    _doIncite(player, RANGE);
    player.sendMessage('§c§lIncitement! §r§7Mobs will fight each other for 15 seconds.');
    player.playSound('mob.ghast.scream', { pitch: 1.2, volume: 0.8 });

    const id = system.runInterval(() => {
        if (!player.isValid() || ticksLeft <= 0) {
            system.clearRun(id);
            activeIncitements.delete(player.id);
...
function _doIncite(player, range) {
    try {
        ...
        for (const mob of targets) {
            ...
            try {
                victim.applyDamage(2, { cause: EntityDamageCause.entityAttack, damagingEntity: mob });
            } catch (_) {}
...
function _castConspiracy(player) {
    ...
    // Brief invisibility + speed — the Conspirer vanishes from perception
    try { player.addEffect('invisibility', 120, { amplifier: 0, showParticles: false }); } catch (_) {}
    try { player.addEffect('speed', 80, { amplifier: 3, showParticles: false }); } catch (_) {}

    // Blind nearby hostiles and leave a false presence behind
    const origPos = { ...player.location };
    try {
        const mobs = dim.getEntities({ location: origPos, maxDistance: 20 })
            .filter(e => e.typeId !== 'minecraft:player' && e.isValid() && _isHostile(e));
        for (const mob of mobs.slice(0, 8)) {
            try { mob.addEffect('blindness', 80, { amplifier: 0, showParticles: true }); } catch (_) {}
            try { mob.addEffect('slowness', 40, { amplifier: 1, showParticles: false }); } catch (_) {}
        }
    } catch (_) {}

    // Purple smoke burst at player's position (the "false presence")
    for (let i = 0; i < 16; i++) {
        const a = (i / 16) * Math.PI * 2;
        try { dim.spawnParticle('minecraft:dragon_breath_trail', {
            x: origPos.x + Math.cos(a) * 0.6, y: origPos.y + 1, z: origPos.z + Math.sin(a) * 0.6
        }); } catch (_) {}
    }
...
function _castFlameTransform(player) {
    ...
    const dim   = player.dimension;
    const SPEED = 0.5, MAX_DIST = 15, STEP = 0.5;
    ...
    player.playSound('mob.blaze.shoot', { pitch: 0.6, volume: 1.0 });
    ...
    const tick = () => {
        ...
        // Crimson-white flame trail
        try { dim.spawnParticle('minecraft:basic_flame_particle',     pos); } catch (_) {}
        try { dim.spawnParticle('minecraft:mobflame_single', { x: pos.x, y: pos.y + 0.1, z: pos.z }); } catch (_) {}

        // Block hit — teleport player here
        let hitPos = null;
        try {
            const block = dim.getBlock({ x: Math.floor(pos.x), y: Math.floor(pos.y), z: Math.floor(pos.z) });
            if (block && !block.isAir && !block.isLiquid) {
                // Land one block above the hit block
                hitPos = { x: pos.x, y: Math.floor(pos.y) + 1, z: pos.z };
            }
        } catch (_) {}

        // Entity hit — burn them and teleport to their position
        if (!hitPos) {
            try {
                ...
                for (const e of near) {
                    ...
                    try { e.applyDamage(15, { damagingEntity: player }); } catch (_) {}
        ...
        if (hitPos) {
            ...
            // Teleport player — they are fire immune so arrive safely
            ...
            try { dim.spawnParticle('minecraft:large_explosion', hitPos); } catch (_) {}
...
function _conjureFlameBlade(player) {
    try {
        ...
        player.sendMessage('§4§lFlame Blade conjured! §r§7Wield it well — it endures for as long as your will holds.');
        player.playSound('fire.fire', { pitch: 0.5, volume: 1.2 });
        // Crimson flash
        for (let i = 0; i < 10; i++) {
            const a = (i / 10) * Math.PI * 2;
            try { player.dimension.spawnParticle('minecraft:basic_flame_particle', {
                x: player.location.x + Math.cos(a) * 0.6, y: player.location.y + 1, z: player.location.z + Math.sin(a) * 0.6
            }); } catch (_) {}
        }
    } catch (_) {}
}
...
function _castFireBolt(player) {
    ...
    const SPEED = 0.8, STEPS = 45, DAMAGE = SPELLS.fire_bolt.damage;
    ...
    if (!bolt) return;
    player.playSound('fire.fire', { pitch: 1.3, volume: 1.0 });
    ...
    const tick = () => {
        ...
        try {
            const block = dim.getBlock({ x: Math.floor(pos.x), y: Math.floor(pos.y), z: Math.floor(pos.z) });
            if (block && !block.isAir && !block.isLiquid) { try { bolt.kill(); } catch(_) {} _fireImpact(dim, pos); return; }
        } catch (_) {}
        // Crimson-white helix trail
        const radius = 0.25;
        for (let j = 0; j < 4; j++) {
            const rad = ((step + j/4)/6.0) * Math.PI * 2;
            const cos = Math.cos(rad)*radius, sin = Math.sin(rad)*radius;
            try { dim.spawnParticle('minecraft:mobflame_single', { x: pos.x + right.x*cos + up2.x*sin, y: pos.y + right.y*cos + up2.y*sin, z: pos.z + right.z*cos + up2.z*sin }); } catch(_) {}
        }
        try {
...
function _castFireball(player) {
    const view = player.getViewDirection(), eye = player.getHeadLocation(), dim = player.dimension;
    const SPEED = 0.6, STEPS = 60, DAMAGE = SPELLS.fireball.damage, AOE = 6;
    const spawnPos = { x: eye.x + view.x*2, y: eye.y, z: eye.z + view.z*2 };
    let fb;
    try { fb = dim.spawnEntity('lotm:firebolt', spawnPos); } catch (_) { return; }
    if (!fb) return;
    const pos = { ...spawnPos };
    let step = 0, vy = view.y + 0.12;
    player.playSound('mob.ghast.fireball', { pitch: 0.9, volume: 1.2 });

    const tick = () => {
        if (!fb.isValid() || step++ >= STEPS) { try { if (fb.isValid()) fb.kill(); } catch(_) {} return; }
        pos.x += view.x*SPEED; pos.y += vy*SPEED; pos.z += view.z*SPEED; vy -= 0.02;
        ...
        if (!blockHit) {
            try {
                ...
                for (const e of near) { if (e.id === player.id || e.id === fb.id) continue; entityHit = true; break; }
            } catch (_) {}
        }
        if (blockHit || entityHit) {
            ...
            for (let j = 0; j < 20; j++) {
                ...
                try { dim.spawnParticle('minecraft:mobflame_single', { x:pos.x+Math.cos(a)*2, y:pos.y, z:pos.z+Math.sin(a)*2 }); } catch(_) {}
            }
            player.playSound('random.explode', { pitch:0.8, volume:1.8 });
            try {
...
function _castFireWall(player) {
    ...
    const center = { x: player.location.x + dir.x*5, y: player.location.y, z: player.location.z + dir.z*5 };
...
function _castBlazingSpear(player) {
    const view = player.getViewDirection(), eye = player.getHeadLocation(), dim = player.dimension;
    const SPEED = 0.6, STEPS = 42, DAMAGE = SPELLS.blazing_spear.damage;
    const start = { x: eye.x+view.x*1.5, y: eye.y, z: eye.z+view.z*1.5 };
    const hLen = Math.sqrt(view.x*view.x + view.z*view.z) || 1;
    ...
    if (!spear) return;
    player.playSound('item.trident.throw', { pitch: 0.55, volume: 1.3 });
    ...
    const tick = () => {
        ...
        for (const [right, up] of offsets) {
            ...
            try { dim.spawnParticle('minecraft:mobflame_single', p); } catch(_) {}
        }
        try {
            ...
            for (const e of near) {
                ...
                try { e.applyDamage(DAMAGE, { damagingEntity:player }); } catch(_) {}
                try { e.setOnFire(8, true); } catch(_) {}
                try { e.applyKnockback(view.x, view.z, 1.0, 0.3); } catch(_) {}
...
function _castRingOfFire(player) {
    ...
    player.playSound('mob.ghast.fireball', { pitch: 0.5, volume: 1.4 });
    for (let wave = 0; wave < 3; wave++) {
        activeRings.push({
            center:      { ...center },
            dimension:   dim,
            player,
            startTime:   Date.now() + wave * 500,
            lastRadius:  -1,
            hitEntities: new Set()
        });
    }
...
function _castFlamingBarrage(player) {
    const dim = player.dimension, RANGE = 24, DAMAGE = SPELLS.flaming_barrage.damage, COUNT = 6;
    let targets;
    try {
        targets = dim.getEntities({ location:player.location, maxDistance:RANGE,
    ...
    targets.sort((a,b) => {
        ...
        return (a.location.x-pl.x)**2+(a.location.z-pl.z)**2 - ((b.location.x-pl.x)**2+(b.location.z-pl.z)**2);
    });
    player.playSound('mob.ghast.fireball', { pitch: 1.3, volume: 1.2 });
...
function _launchHomingFireball(player, target, damage) {
    const eye=player.getHeadLocation(), dim=player.dimension, SPEED=0.55, HOMING=0.18;
    const tLoc=target.location;
    const dx0=tLoc.x-eye.x, dy0=(tLoc.y+1)-eye.y, dz0=tLoc.z-eye.z;
    const len0=Math.sqrt(dx0*dx0+dy0*dy0+dz0*dz0)||1;
    const inaccuracy=Math.min(0.2,len0/80);
    ...
    player.playSound('mob.blaze.shoot', { pitch:1.2+Math.random()*0.3, volume:0.9 });
    const tick = () => {
        if (!fb.isValid()||step++>=70) { try { if(fb.isValid())fb.kill(); } catch(_) {} return; }
        if (target.isValid()) {
            const tp=target.location;
            const tdx=tp.x-pos.x, tdy=(tp.y+1)-pos.y, tdz=tp.z-pos.z;
            const tLen=Math.sqrt(tdx*tdx+tdy*tdy+tdz*tdz)||1;
            const tx=tdx/tLen*SPEED, ty=tdy/tLen*SPEED, tz=tdz/tLen*SPEED;
            velX=velX*(1-HOMING)+tx*HOMING; velY=velY*(1-HOMING)+ty*HOMING; velZ=velZ*(1-HOMING)+tz*HOMING;
            const vLen=Math.sqrt(velX*velX+velY*velY+velZ*velZ)||1;
            velX=velX/vLen*SPEED; velY=velY/vLen*SPEED; velZ=velZ/vLen*SPEED;
        }
        ...
        if (!hit) {
            try {
                ...
                for (const e of near) { if (e.id===player.id||e.id===fb.id) continue; hit=true; hitEntity=e; break; }
            } catch(_) {}
        }
        if (hit) {
            try { fb.kill(); } catch(_) {}
            ...
            if (hitEntity) {
                ...
                try { hitEntity.setOnFire(8, true); } catch(_) {}
            }
            return;
        }
        system.runTimeout(tick, 1);
...
function _castBurningDash(player) {
    const dim=player.dimension, STEPS=30, DAMAGE=SPELLS.burning_dash.damage;
    try { player.addEffect('speed', 40, { amplifier: 4, showParticles:false }); } catch(_) {}
    player.playSound('mob.blaze.burn', { pitch:1.4, volume:1.2 });
    ...
    const id=system.runInterval(() => {
        ...
        for (let i=0;i<6;i++) {
            ...
            try { dim.spawnParticle('minecraft:mobflame_single', { x:pos.x+ox, y:pos.y+Math.random()*1.8, z:pos.z+oz }); } catch(_) {}
        }
        try {
            const near=dim.getEntities({ location:pos, maxDistance:1.8,
                excludeTypes:['minecraft:item','minecraft:xp_orb','lotm:wisp','lotm:firebolt','lotm:flame_spear','lotm:fire_arrow'] });
            for (const e of near) {
                if (!e.isValid()||e.id===player.id||hit.has(e.id)) continue;
                hit.add(e.id);
                try { e.applyDamage(DAMAGE, { damagingEntity:player }); } catch(_) {}
                try { e.setOnFire(6, true); } catch(_) {}
...
function _castFireArrowBlazeStorm(player) {
    ...
    player.playSound('mob.blaze.shoot', { pitch:1.1, volume:1.2 });
    const hLen=Math.sqrt(view.x*view.x+view.z*view.z)||1;
    const rightX=-view.z/hLen, rightZ=view.x/hLen;
    for (let i=0;i<6;i++) {
        system.runTimeout(() => {
            const sH=(Math.random()-0.5)*0.6, sV=(Math.random()-0.5)*0.4;
            const dir={ x:view.x+rightX*sH, y:view.y+sV, z:view.z+rightZ*sH };
            const dLen=Math.sqrt(dir.x*dir.x+dir.y*dir.y+dir.z*dir.z)||1;
            _launchBlazeBolt(player, { x:dir.x/dLen, y:dir.y/dLen, z:dir.z/dLen }, eye, SPELLS.fire_arrow_blaze.damage * 0.35);
        }, 5+i*2);
    }
}

function _launchFireArrow(player, view, eye) {
    const dim=player.dimension, SPEED=0.9, GRAV=0.016, STEPS=60, DAMAGE=SPELLS.fire_arrow_blaze.damage;
    const start={ x:eye.x+view.x*1.5, y:eye.y, z:eye.z+view.z*1.5 };
    let arrow;
    try { arrow=dim.spawnEntity('lotm:fire_arrow', start); } catch(_) { return; }
    if (!arrow) return;
    player.playSound('item.trident.throw', { pitch:1.15, volume:1.0 });
    const yaw=Math.atan2(-view.x, view.z)*(180/Math.PI);
    ...
    const tick = () => {
        ...
        try {
            const block=dim.getBlock({ x:Math.floor(pos.x), y:Math.floor(pos.y), z:Math.floor(pos.z) });
            if (block&&!block.isAir&&!block.isLiquid) { try { arrow.kill(); } catch(_) {} _fireImpact(dim,pos); return; }
        } catch(_) {}
        try {
            const near=dim.getEntities({ location:{ x:pos.x, y:pos.y-0.5, z:pos.z }, maxDistance:1.3,
...
function _launchBlazeBolt(player, dir, eye, damage) {
    const dim=player.dimension, SPEED=0.75, STEPS=40;
    ...
    const tick = () => {
        ...
        try { bolt.teleport(pos); } catch(_) {}
        const a=step*0.6;
        try { dim.spawnParticle('minecraft:mobflame_single', { x:pos.x+Math.cos(a)*0.25, y:pos.y, z:pos.z+Math.sin(a)*0.25 }); } catch(_) {}
        try {
            const block=dim.getBlock({ x:Math.floor(pos.x), y:Math.floor(pos.y), z:Math.floor(pos.z) });
            if (block&&!block.isAir&&!block.isLiquid) { try { bolt.kill(); } catch(_) {} return; }
        } catch(_) {}
        try {
            const near=dim.getEntities({ location:pos, maxDistance:1.0, excludeTypes:['minecraft:item','lotm:wisp','lotm:firebolt','lotm:fire_arrow'] });
            for (const e of near) {
                if (!e.isValid()||e.id===player.id||hit.has(e.id)) continue;
                hit.add(e.id);
                try { e.applyDamage(damage, { damagingEntity:player }); } catch(_) {}
...
function _fireImpact(dim, pos) {
    ...
    for (let j=0;j<12;j++) {
        ...
        try { dim.spawnParticle('minecraft:mobflame_single', { x:pos.x+Math.cos(a)*0.5, y:pos.y, z:pos.z+Math.sin(a)*0.5 }); } catch(_) {}
    }
}
